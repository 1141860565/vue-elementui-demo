<template>
  <div id="topdiv">
    <div id="buttondiv">
        <!-- 查询，新增按钮 -->
      <button id="buttonQry" v-text="butttonQry" v-on:click="qrysrcsys"></button>
      <!-- <button id="buttonQry" v-text="butttonUpdate" v-on:click></button>
      <button id="buttonQry" v-text="butttonDel" v-on:click></button>-->
      <button id="buttonQry" v-text="butttonAdd" v-on:click="dialogFormVisibleAdd = true"></button>
    </div>
    ...
    <el-dialog title="新增" :visible.sync="dialogFormVisibleAdd" @closed="dialogFormVisibleAddBeforeClose" >
      <el-form :model="formAdd">
        <el-form-item label="sys" :label-width="formLabelWidth" required ="true">
          <el-input v-model="formAdd.sys" autocomplete="off" ></el-input>
        </el-form-item>
        <el-form-item label="sysNum" :label-width="formLabelWidth" required ="true">
        ...
        <el-form-item label="dbSid" :label-width="formLabelWidth" required ="true">
          <el-input v-model="formAdd.dbSid" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="dbSchema" :label-width="formLabelWidth" required ="true">
...
export default {
  ...
  methods: {
    ...
    tijiaoForm (value) {
      this.$http.post('/api/srcsys/updatesrcsys', value).then(res => {
        // 改动之后重新查询
        this.qrysrcsys()
    ...
    tijiaoformAdd (value) {
      this.$http.post('/api/srcsys/addsrcsys', value).then(res => {
        // 改动之后重新查询
        this.qrysrcsys()
    ...
    handleDelete (index, row) {
      ...
      this.$http.post('/api/srcsys/delsrcsys', row).then(res => {
        this.qrysrcsys()
